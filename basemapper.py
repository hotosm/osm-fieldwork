...
def process_geojson(geojson_content):
    """Process the GeoJSON content and extract the bounding box.

    Args:
        geojson_content (str): Content of the GeoJSON file as a string.

    Returns:
        tuple: Bounding box coordinates (minx, miny, maxx, maxy).
    """
...
def create_basemap_file(
    ...
    if isinstance(boundary, io.BytesIO):
        ...
        if len(bbox_parts) == 4:
            ...
            err = "Bounding box string must have 4 coordinates separated by commas (min_x, min_y, max_x, max_y)"
            log.error(err)
            raise ValueError(err)
    else:
