...
def merge_sheets(mandatory_df, custom_df, digitisation_df):
        ...
        common_fields = set(custom_df['name']).intersection(set(mandatory_df['name'])).union(set(custom_df['name']).intersection(set(digitisation_df['name'])))
        ...
        merged_df = pd.concat([custom_common_df, mandatory_df_filtered, custom_non_common_df, digitisation_df_filtered], ignore_index=True)
...
def update_xls_form(custom_form:BytesIO):
    ...
    if 'choices' in mandatory_sheets and 'choices' in digitisation_sheets and 'choices' in custom_sheets:
        custom_sheets['choices'] = merge_sheets(mandatory_sheets['choices'], custom_sheets['choices'], digitisation_sheets['choices'])
    ...
    if 'entities' in mandatory_sheets:
        if 'entities' in custom_sheets:
            custom_sheets['entities'] = pd.concat([custom_sheets['entities'], mandatory_sheets['entities']], ignore_index=True).drop_duplicates()
        else:
